...
class BasicParser:
    ...
    copart_headers = {
        ...
        "x-xsrf-token": "52f356dd-7094-47bd-a259-3e0f57c91ccb",
    ...
    async def get_json(
        ...
        async with self._session.get(url, ssl=False, headers=self.headers) as response:
    ...
    async def get_copart_json(self, copart_url: str, referer: str):
        ...
        try:
            async with self._session.get(
                copart_url, ssl=False, headers=self.copart_headers
            ) as response:
                return await response.json()
        except:
            return False
